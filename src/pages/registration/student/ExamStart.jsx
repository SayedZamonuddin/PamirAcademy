...
const FALLBACK_SUBJECTS = [
  { name: "English", time: "60 Min" },
  { name: "Physics", time: "60 Min" },
  { name: "Math", time: "60 Min" },
];
...
export default function ExamStart() {
  ...
  const subjects = useMemo(() => {
    try {
      ...
      if (!Array.isArray(saved) || saved.length === 0) return FALLBACK_SUBJECTS;
      return saved.map((name) => ({ name, time: "60 Min" }));
    } catch {
      return FALLBACK_SUBJECTS;
    }
  }, []);

  const handleStartSubject = (subjectName) => {
    try {
      localStorage.setItem("currentExamSubject", subjectName);
    } catch (_) {}
    navigate("/register/student/exam-in-progress");
  };

  const handlePrevious = () => {
    navigate("/register/student/subjects");
  };
      ...
      <main className="flex-1 px-[clamp(24px,5vw,80px)] pb-10">
        <div className="max-w-[1200px] mx-auto">
          ...
          <div className="flex items-center justify-between mt-12">
            ...
            <button
              onClick={handleNext}
              className="flex items-center gap-2 bg-[#7d807f] text-white px-8 py-3.5 rounded-full border-none cursor-pointer tracking-wider"
            >
              NEXT <ChevronRight />
            </button>
          </div>
