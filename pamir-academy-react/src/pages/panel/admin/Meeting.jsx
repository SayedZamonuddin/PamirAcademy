...
const TEACHER_IMG = "https://images.unsplash.com/photo-1574132190990-cfd62178bb1c?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHx5b3VuZyUyMG1hbGUlMjB0ZWFjaGVyJTIwcG9ydHJhaXQlMjBjbGFzc3Jvb218ZW58MXx8fHwxNzczOTQzNjEwfDA&ixlib=rb-4.1.0&q=80&w=1080";
const ADMIN_IMG = "https://images.unsplash.com/photo-1672685667592-0392f458f46f?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxwcm9mZXNzaW9uYWwlMjBtYW4lMjBwb3J0cmFpdCUyMGhlYWRzaG90fGVufDF8fHx8MTc3Mzg3MzAwNXww&ixlib=rb-4.1.0&q=80&w=1080";
...
const CheckIcon = () => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>;
const XIcon = () => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>;
...
const BREADCRUMBS = ["Demo","English","Elementary","Grammar","Past Simple"];
...
const INITIAL_MESSAGES = [
  ...
  { id: 2, sender: "Applicant", text: "Why are you?", isAdmin: false },
  { id: 3, sender: "Admin", text: "I am doing well", isAdmin: true },
  { id: 4, sender: "Applicant", text: "Great", isAdmin: false },
];
...
export default function TeacherDemo() {
  ...
  const handleDecision = (type) => { setDecision(type === "accept" ? "accepted" : "refused"); setShowConfirm(null); };

  const CONTROLS = [
    { icon: <CameraIcon/>, bg: "bg-white/20", label: "Camera" },
    { icon: <CameraOffIcon/>, bg: "bg-white/20", label: "Screen" },
    { icon: <MicIcon/>, bg: "bg-white/20", label: "Mic" },
    { icon: <ScreenShareIcon/>, bg: "bg-white/20", label: "Share" },
    { icon: <EndCallIcon/>, bg: "bg-[#c51310]", label: "End" },
  ];
  ...
  return (
    <DashboardLayout activePage="demo">
      <div className="flex-1 flex flex-col overflow-y-auto">
        {/* Breadcrumb */}
        <div className="mx-[clamp(12px,2vw,24px)] mt-[clamp(12px,2vw,24px)] bg-[#006236] rounded-full py-2.5 px-[clamp(16px,2vw,32px)] flex items-center justify-center gap-1.5 flex-wrap">
        ...
        <div className="flex-1 p-[clamp(12px,2vw,24px)] grid gap-[clamp(12px,1.5vw,20px)]" style={{ gridTemplateColumns: "minmax(260px, 1fr) minmax(300px, 1.4fr)", gridTemplateRows: "auto 1fr" }}>
          {/* Lesson Plan */}
          <div className="row-span-2 bg-[#d9d9d9] rounded-2xl p-[clamp(16px,2vw,28px)] flex flex-col gap-5 overflow-y-auto">
          ...
          <div className="bg-[#1a1a1a] rounded-2xl relative overflow-hidden min-h-[clamp(200px,28vw,380px)]">
            <div className="absolute top-3 right-3 bg-[#006236] text-white px-4 py-1.5 rounded-full text-[clamp(12px,1.1vw,16px)] font-semibold z-2 flex items-center gap-1.5">
            ...
            <div className="absolute bottom-16 left-3 w-[clamp(80px,10vw,130px)] h-[clamp(60px,7.5vw,100px)] rounded-xl overflow-hidden border-2 border-[#006236] z-2">
              <img src={ADMIN_IMG} alt="Admin" className="w-full h-full object-cover"/>
            </div>
          ...
          <div className="flex flex-col gap-3 min-h-[200px]">
            <div className="flex items-center gap-2.5 bg-[#d9d9d9] rounded-full py-2 pr-4 pl-6">
              <input type="text" value={inputText} onChange={e => setInputText(e.target.value)} placeholder="Enter you text to chat..."
                className="flex-1 border-none outline-none bg-transparent text-[clamp(13px,1.2vw,18px)] text-black"/>
              <button className="bg-transparent border-none cursor-pointer flex items-center p-1.5"><SendIcon/></button>
            </div>
            <div className="flex-1 bg-[#d9d9d9] rounded-2xl p-[clamp(12px,1.5vw,20px)] flex flex-col gap-3 overflow-y-auto">
              {messages.map(msg => (
        ...
        <div className={`mx-[clamp(12px,2vw,24px)] mb-[clamp(12px,2vw,24px)] rounded-2xl px-[clamp(20px,3vw,40px)] py-[clamp(16px,2vw,24px)] flex items-center justify-between flex-wrap gap-4 transition-all duration-300
          ${decision ? (decision === "accepted" ? "bg-[#006236]/15 border-2 border-[#006236]" : "bg-[#c51310]/15 border-2 border-[#c51310]") : "bg-black/25 border-2 border-transparent"}`}>

          {decision ? (
            <div className="flex items-center gap-3 flex-1 justify-center">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center ${decision === "accepted" ? "bg-[#006236]" : "bg-[#c51310]"}`}>
                {decision === "accepted" ? <CheckIcon/> : <XIcon/>}
              </div>
              <span className="text-white text-[clamp(16px,1.8vw,24px)] font-semibold">
                {decision === "accepted" ? "Teacher Accepted — Offer letter will be sent." : "Teacher Refused — Applicant will be notified."}
              </span>
              <button onClick={() => setDecision(null)} className="ml-auto bg-transparent text-white border border-white/40 rounded-full px-5 py-2 cursor-pointer text-[clamp(12px,1vw,15px)]">Undo</button>
            </div>
          ) : showConfirm ? (
            <div className="flex items-center gap-4 flex-1 justify-center flex-wrap">
              <span className="text-white text-[clamp(14px,1.4vw,20px)]">
              ...
              <button onClick={() => setShowConfirm(null)} className="bg-transparent text-white px-7 py-2.5 rounded-full border border-white/50 cursor-pointer text-[clamp(14px,1.3vw,18px)]">Cancel</button>
            </div>
          ) : (
            <>
              <div className="text-white text-[clamp(13px,1.2vw,18px)] opacity-80">After observing the demo lesson, make your decision:</div>
