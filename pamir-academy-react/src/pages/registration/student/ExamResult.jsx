      ...
      <main className="flex-1 px-[clamp(24px,5vw,80px)] pb-10">
        <div className="max-w-[1200px] mx-auto">
          ...
          <div className="flex flex-col items-center">
            {SUBJECTS.map((subject, idx) => (
              <div key={subject.name} className="w-full">
                  ...
                  <div className="flex items-center justify-center gap-[clamp(24px,4vw,60px)] mb-6 flex-wrap">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <div className="bg-[#d9d9d9] rounded-[10px] px-5 py-1">
                        <span className="text-brand text-[clamp(12px,1.1vw,18px)] font-semibold">{subject.level}</span>
                      </div>
                    </div>
                    <button className="bg-brand text-white px-7 py-2.5 rounded-full border-none cursor-pointer text-[clamp(14px,1.2vw,20px)] tracking-wide whitespace-nowrap">Re-Take</button>
                  </div>
                ) : (
                  <div className="flex justify-center mb-6">
                    <button className="w-[clamp(130px,12vw,180px)] h-[clamp(130px,12vw,180px)] rounded-full bg-brand border-none cursor-pointer flex flex-col items-center justify-center gap-0.5 shadow-lg">
                ...
                {idx < SUBJECTS.length - 1 && <div className="w-full h-1 bg-brand rounded-sm my-4 mb-8"/>}
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between mt-12">
            <button className="flex items-center gap-2 bg-brand text-white px-8 py-3.5 rounded-full border-none cursor-pointer tracking-wider"><ChevronLeft/> PREVIOUS</button>
