  ...
  return (
    <div className="w-screen min-h-screen flex flex-col bg-surface overflow-x-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* ---- BREADCRUMB + STATUS ---- */}
        <div className="mx-[clamp(12px,2vw,24px)] mt-[clamp(12px,2vw,24px)] bg-brand rounded-full py-2.5 px-[clamp(16px,2vw,32px)] flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex-1 p-[clamp(12px,2vw,24px)] grid gap-[clamp(12px,1.5vw,20px)] overflow-hidden" style={{ gridTemplateColumns: "minmax(280px, 1fr) minmax(320px, 1.6fr)", gridTemplateRows: "1fr" }}>
          ...
          <div className="flex flex-col gap-0 overflow-hidden rounded-2xl bg-[#d9d9d9]">
            {/* Tab bar */}
            <div className="flex border-b-2 border-brand/15 bg-[#d9d9d9] shrink-0">
              {TAB_ITEMS.map(tab => (
                <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                  className={`flex-1 flex items-center justify-center gap-1 py-3 text-[clamp(10px,1vw,13px)] cursor-pointer border-none transition-colors ${
                    activeTab === tab.key
                      ? "bg-white text-brand font-semibold border-b-2 border-brand"
                      : "bg-transparent text-gray-500 hover:text-brand"
                  }`}>
                  {tab.icon}
            ...
            <div className="flex-1 overflow-y-auto p-[clamp(14px,2vw,24px)]">
              ...
              {activeTab === "plan" && (
                <div className="flex flex-col gap-4">
                  <h2 className="text-brand text-[clamp(18px,2vw,26px)] font-bold text-center m-0 pb-3 border-b-2 border-brand/20">
                    {LESSON_PLAN.title}
                  </h2>
                  {/* Objectives */}
                  <div className="bg-brand/10 rounded-xl p-3">
                    <h4 className="text-brand text-sm font-bold m-0 mb-2">Learning Objectives</h4>
                  ...
                  {LESSON_PLAN.sections.map((section, idx) => (
                    <div key={idx} className="flex flex-col gap-2">
                      <h3 className="text-brand text-[clamp(14px,1.4vw,18px)] font-bold m-0">{section.heading}</h3>
                      ...
                      <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[clamp(10px,0.9vw,12px)] self-start ${
                        section.visibility === "teacher" ? "bg-[#c51310]/10 text-[#c51310]" : "bg-brand/10 text-brand"
                      }`}>
                        {section.visibility === "teacher" ? <LockIcon/> : <EyeIcon/>}
                        <span>{section.visibility === "teacher" ? "Only you can see" : "Visible to proctor"}</span>
                      </div>
                      {idx < LESSON_PLAN.sections.length - 1 && <div className="h-px bg-brand/15 mt-1"/>}
                    </div>
                  ))}
                </div>
              )}
              ...
              {activeTab === "chat" && (
                <div className="flex flex-col gap-3 h-full">
                    ...
                    {messages.map(msg => (
                      <div key={msg.id} className={`flex flex-col ${msg.isProctor ? "items-start" : "items-end"}`}>
                        ...
                        <div className={`${msg.isProctor
                          ? "bg-brand rounded-[20px_20px_20px_4px]"
                  ...
                  <div className="flex items-center gap-2 bg-white rounded-full py-2 pr-3 pl-5 shrink-0 border border-brand/20">
                    <input type="text" value={inputText} onChange={e => setInputText(e.target.value)}
                      onKeyDown={e => { if (e.key === "Enter") sendMessage(); }}
                      placeholder="Type a message..."
                      className="flex-1 border-none outline-none bg-transparent text-[clamp(13px,1.1vw,16px)] text-black"/>
                    <button onClick={sendMessage} className="bg-transparent border-none cursor-pointer flex items-center p-1 text-brand"><SendIcon/></button>
                  </div>
                </div>
              )}
              ...
              {activeTab === "notes" && (
                <div className="flex flex-col gap-3 h-full">
                  ...
                  <textarea value={notes} onChange={e => setNotes(e.target.value)}
                    className="flex-1 w-full px-4 py-3 rounded-xl border-2 border-brand/20 text-sm outline-none bg-white focus:border-brand/50 resize-none min-h-[200px] transition-colors"
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => { localStorage.setItem("demoSessionNotes", notes); }}
                      className="px-4 py-1.5 rounded-full bg-brand text-white text-xs cursor-pointer border-none hover:bg-brand-dark transition-colors">
                      Save Notes
                    </button>
                  </div>
                </div>
              ...
              {activeTab === "checklist" && (
                <div className="flex flex-col gap-4">
                  <div className="flex items-center justify-between">
                    <h3 className="m-0 text-brand text-sm font-bold">Pre-Session Checklist</h3>
                    ...
                    {checklist.map(item => (
                      <button key={item.id} onClick={() => toggleChecklist(item.id)}
                        className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer border-none text-left transition-colors ${
                          item.done ? "bg-brand/10" : "bg-white hover:bg-gray-50"
                        }`}>
                        <span className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${
                          item.done ? "bg-brand text-white" : "border-2 border-gray-300"
                        }`}>
                          {item.done && <CheckCircleIcon/>}
                        </span>
                        <span className={`text-[clamp(13px,1.1vw,15px)] ${item.done ? "text-brand line-through opacity-70" : "text-gray-700"}`}>
                          {item.text}
                        </span>
                      </button>
                    ))}
                  ...
                  {allChecked && (
                    <div className="bg-brand/10 border border-brand/20 rounded-xl p-3 text-center text-brand text-sm font-semibold flex items-center justify-center gap-2">
            ...
            {sessionState === "waiting" && (
              <div className="flex-1 bg-[#1a1a1a] rounded-2xl flex flex-col items-center justify-center gap-5 p-8 text-center relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col items-center gap-4">
                  <div className="w-20 h-20 rounded-full border-4 border-brand overflow-hidden">
                  ...
                  <div className="bg-white/10 rounded-xl px-6 py-4 flex flex-col gap-1.5">
                    <div className="flex items-center gap-2 text-gray-300 text-sm">
                      <span className="text-brand font-semibold">Subject:</span> {SESSION_INFO.subject} — {SESSION_INFO.level}
                    </div>
                    <div className="flex items-center gap-2 text-gray-300 text-sm">
                      <span className="text-brand font-semibold">Topic:</span> {SESSION_INFO.subtopic} — {SESSION_INFO.topic}
                    </div>
                    <div className="flex items-center gap-2 text-gray-300 text-sm">
                      <span className="text-brand font-semibold">Date:</span> {SESSION_INFO.date}
                    </div>
                    <div className="flex items-center gap-2 text-gray-300 text-sm">
                      <span className="text-brand font-semibold">Time:</span> {SESSION_INFO.scheduledTime}
                    </div>
                    <div className="flex items-center gap-2 text-gray-300 text-sm">
                      <span className="text-brand font-semibold">Duration:</span> 40 minutes
                    </div>
                  </div>

                  <button onClick={startSession}
                    className="mt-2 flex items-center gap-2 bg-brand text-white px-8 py-3.5 rounded-full border-none cursor-pointer text-base font-bold tracking-wider hover:bg-brand-dark transition-colors hover:scale-[1.03]">
              ...
              <>
                ...
                <div className="flex-1 bg-[#1a1a1a] rounded-2xl relative overflow-hidden min-h-[clamp(220px,30vw,400px)]">
                  ...
                  <div className="absolute top-3 right-3 z-10 flex items-center gap-2">
                    ...
                    <div className="bg-brand text-white px-4 py-1.5 rounded-full text-[clamp(12px,1.1vw,16px)] font-semibold flex items-center gap-1.5">
                  ...
                  {cameraOn ? (
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-800">
                      <div className="w-20 h-20 rounded-full bg-brand flex items-center justify-center text-white text-3xl font-bold">T</div>
                      <p className="text-gray-400 text-sm mt-3">Camera is off</p>
                    </div>
                  )}

                  {/* Proctor PiP */}
                  <div className="absolute bottom-16 left-3 w-[clamp(80px,10vw,140px)] h-[clamp(60px,7.5vw,105px)] rounded-xl overflow-hidden border-2 border-brand z-10 shadow-lg">
                    <img src={PROCTOR_IMG} alt="Proctor" className="w-full h-full object-cover"/>
                ...
                <div className="bg-[#d9d9d9] rounded-xl px-4 py-3 flex items-center gap-3 shrink-0">
                  <span className="text-brand text-xs font-semibold whitespace-nowrap">{formatTime(timeElapsed)}</span>
                  <div className="flex-1 h-2.5 bg-gray-300 rounded-full overflow-hidden relative">
                    <div className="h-full bg-brand rounded-full transition-all duration-1000" style={{ width: `${progress}%` }}/>
                    {/* Warning zone */}
            ...
            {sessionState === "ended" && (
              <div className="flex-1 bg-[#1a1a1a] rounded-2xl flex flex-col items-center justify-center gap-5 p-8 text-center">
                ...
                <button onClick={() => { setSessionState("waiting"); setTimeElapsed(0); }}
                  className="mt-2 bg-brand text-white px-6 py-2.5 rounded-full border-none cursor-pointer text-sm font-semibold hover:bg-brand-dark transition-colors">
                  Back to Lobby
                </button>
              </div>
            )}
        ...
        {sessionState === "live" && (
          <div className="mx-[clamp(12px,2vw,24px)] mb-[clamp(12px,2vw,24px)] bg-brand/10 border-2 border-brand/20 rounded-2xl px-[clamp(16px,2vw,32px)] py-3 flex items-center justify-between flex-wrap gap-3">
            ...
            <div className="flex items-center gap-4 text-xs text-gray-500">
              <span className="flex items-center gap-1">
                <span className={`w-2 h-2 rounded-full ${cameraOn ? "bg-brand" : "bg-[#c51310]"}`}/>
                Camera {cameraOn ? "On" : "Off"}
              </span>
              <span className="flex items-center gap-1">
                <span className={`w-2 h-2 rounded-full ${micOn ? "bg-brand" : "bg-[#c51310]"}`}/>
                Mic {micOn ? "On" : "Off"}
              </span>
            </div>
          </div>
