...
import { getMyGroups, getAvailableGroups, requestGroupChange } from "../../../utils/panelApi";
...
export default function StudentGroups() {
  ...
  useEffect(() => {
    Promise.all([
      getMyGroups().catch(() => []),
      getAvailableGroups().catch(() => []),
    ]).then(([groups, available]) => {
      setMyGroups(Array.isArray(groups) ? groups : groups.results || []);
      setAvailableGroups(Array.isArray(available) ? available : available.results || []);
    ...
    <StudentLayout activePage="s-groups">
      <div className="flex-1 p-[clamp(16px,3vw,40px)] overflow-y-auto flex flex-col gap-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[
            { label: "Total Classes", value: myGroups.length, icon: <UsersIcon/>, color: "bg-[#006236]" },
            { label: "Group Classes", value: myGroups.filter(g => g.group_type === "group").length, icon: <UsersIcon/>, color: "bg-blue-500" },
            { label: "Individual", value: myGroups.filter(g => g.group_type === "individual").length, icon: <UserIcon/>, color: "bg-amber-500" },
          ].map((card, i) => (
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={`inline-block px-3 py-0.5 rounded-full text-[10px] font-semibold ${
                    group.group_type === "group" ? "bg-[#006236]/15 text-[#006236]" : "bg-amber-100 text-amber-600"
                  }`}>{group.group_type === "group" ? "Group" : "Individual"}</span>
                </div>
                <p className="text-gray-500 text-xs m-0 flex items-center gap-3">
                  <span>Teacher: {group.teacher_name}</span>
                  <span className="flex items-center gap-1"><ClockIcon/> {group.schedule_description}</span>
                </p>
              </div>
              ...
              {(group.members || []).map((m, idx) => (
                <div key={idx} className="bg-white rounded-xl px-5 py-3 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-[#006236] flex items-center justify-center text-white font-bold text-sm border-2 border-[#006236]">
                      {(m.display_name || m.email || "?").split(" ").map(w => w[0]).join("").slice(0, 2).toUpperCase()}
                    </div>
                    <span className="text-[#006236] font-semibold text-sm">{m.display_name || m.email}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            {!submitted ? (
              <>
                <div className="bg-[#006236]/5 rounded-xl px-4 py-3">
                  ...
                  <p className="text-gray-500 text-xs m-0 mt-1">{selectedGroup.teacher_name} · {selectedGroup.schedule_description}</p>
                </div>
                        ...
                        <div key={ag.id} onClick={() => setTargetGroup(ag.id)}
                          ...
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-gray-400 text-xs m-0 mt-0.5">{ag.teacher_name} · {ag.schedule_description}</p>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="text-xs text-gray-400">{ag.spots_left} spots left</span>
